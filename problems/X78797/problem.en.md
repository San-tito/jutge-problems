...
`height, width`, where `width` is an even number, generates zig-zags
like the ones shown in the examples.

## Input

An arbitrary number of cases, where each consists of two positive
natural numbers `height, width`, the `width` being an even number, in
one line.
