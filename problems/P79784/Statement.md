...
a movement ('`n`' for north, '`s`' for south, '`e`' for east, and '`w`'
for west), and that computes the final position of an object initially
...
Input consists of a sequence of characters '`n`', '`s`', '`e`', or
'`w`'.
